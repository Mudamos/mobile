...
class MobileApi {
  ...
  private func handleResponse(response: DataResponse<Any>, completionHandler: ([String: Any]?, MUDError?) -> ()) {
    ...
    case .success:
      ...
      if let status = value["status"] as? String, status == "fail" {
        if let data = value["data"] as? [String: Any], let identifier = data["message"] as? String {
          return completionHandler(nil, .Fail(identifier))
        }

        return completionHandler(nil, .Fail(nil))
      }
