...
const validationMessageFor = (validations) =>
...
const errorMessageFor = ({ payload, defaultMessage }) => {
  ...
  return (
    message || error.userMessage || defaultMessage || locale.errors.genericError
  );
};
...
function appError({ type, payload }) {
  ...
  switch (type) {
    ...
    case "WALLET_CREATE_ERROR":
      return handleWithPayload({
        defaultMessage: locale.errors.walletCreationError,
      });
    case "FACEBOOK_LOGIN_ERROR":
      return handleWithPayload({
        defaultMessage: locale.errors.facebookLoginError,
      });
    ...
    case "PASSWORD_RETRIEVE_ERROR":
      return handleWithPayload({
        defaultMessage: locale.errors.passwordRetrieveError,
      });
    ...
    case "PROFILE_VALIDATE_COMPLETED": {
      return payload.error
        ? handleWithPayload({ defaultMessage: locale.errors.genericError })
        : null;
    }
    ...
    case "SHARE_LINK_ERROR":
      return handleWithPayload({
        defaultMessage: locale.errors.shareLinkError,
      });
    ...
    case "VOTE_CONFIRMATION_SEND_PHONE_ERROR": {
      return handleWithPayload({
        defaultMessage: locale.errors.sendVoteConfirmationError,
      });
    }
    default:
