...
const signButtonTextStyle = { fontFamily: "lato" };
...
class PlipLayout extends Component {
  state = {
    ...
    isValidProfileModalVisible: !!this.props.revalidateProfileSignPlip,
  ...
  onShowSignModal = () => {
    this.setState({
      isSignModalVisible: true,
      isValidProfileModalVisible: false,
    });
  };

  onToggleValidProfileModal = () => {
    this.setState(({ isValidProfileModalVisible }) => ({
      isValidProfileModalVisible: !isValidProfileModalVisible,
    }));
  };
  ...
  render() {
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        />
        <ValidProfileModal
          isVisible={isValidProfileModalVisible}
          onToggleModal={this.onToggleValidProfileModal}
          onOk={this.onShowSignModal}
        />
      </SafeAreaView>
    );
  }
  ...
  renderMainContent() {
    const { signers, signersTotal, onOpenURL, userSignDate } = this.props;
  ...
  signButtonOnPress = ({ canSign, sameRegion, shouldLogin }) => {
    const { onLogin, onRedirectToCantSign } = this.props;
  ...
  renderSignButton() {
    const { userSignDate, user, plip } = this.props;

    const hasSigned = !!userSignDate;
    const logged = !!user;
    const sameRegion =
      (logged && !!eligibleToSignPlip({ plip, user })) || !logged;

    const shouldLogin = !logged;
    const canSign = logged && !hasSigned && sameRegion;

    const title = this.signButtonTitle({
      canSign,
      plip,
      sameRegion,
      shouldLogin,
    });
    const onPress = this.signButtonOnPress({
      canSign,
      sameRegion,
      shouldLogin,
    });
  ...
  renderVideo() {
    ...
    return (
      <YouTube videoId={plip.videoId} style={[styles.video, { height }]} />
    );
  }
  ...
  renderScrollViewHeader() {
    ...
    return (
      <Animated.View
        style={[
          styles.scrollViewHeaderContainer,
          {
            opacity: titlesOpacity,
          },
        ]}>
        <View style={styles.mainTitleContainer}>
          <Text style={styles.mainTitle}>{this.plipName}</Text>
  ...
  renderSignaturesCount() {
    const { plipSignInfo, plip } = this.props;
    ...
    return (
      <View style={styles.signaturesAndGoalContainer}>
        <Text style={styles.signaturesAndGoal}>
          {locale.signaturesAndGoal(signaturesAndGoal)}
        </Text>
      </View>
    );
  }
  ...
  renderPresentation() {
    return (
      <View style={styles.textContainer}>
        <Text numberOfLines={0} style={styles.presentation}>
  ...
  renderButtonReadFullText() {
    return (
      <TouchableOpacity onPress={this.onViewPlip}>
        <View style={styles.actionFullText}>
          <Text style={styles.actionTitle}>
            {locale.readFullText.toUpperCase()}
          </Text>
        </View>
      </TouchableOpacity>
    );
  ...
  renderButtonDownloadPDF() {
    ...
    const subtitle =
      plipSignInfo &&
      plipSignInfo.updatedAt &&
      locale.registeredAt +
        plipSignInfo.updatedAt.format("DD/MM/YYYY [às] HH:mm:ss");
  ...
  renderButtonSignerList() {
    ...
    const title =
      remoteConfig &&
      remoteConfig.authenticatedSignersButtonTitle.toUpperCase();
    const subtitle = locale.youWillBeRedirectToMudamos;
    const action = this.onOpenURL;
    const icon = "exit-to-app";

    return (
      <View style={styles.buttonSignerListContainer}>
        <RoundedButton title={title} action={action} icon={icon} />
        <Text style={styles.buttonInfo}>{subtitle}</Text>
      </View>
    );
  ...
  renderNavBar() {
    ...
    return (
      <NavigationBar
        ...
        rightView={
          !errorFetching && !errorHandlingAppLink && !isFetchingPlipRelatedInfo
            ? this.renderShareButton()
            : null
        }
      />
  ...
  renderFavoriteButton() {
    const { user, plip, plipsFavoriteInfo } = this.props;
  ...
  renderShareButton() {
    return (
      <TouchableOpacity onPress={this.onShare}>
        <Icon name="share" size={24} color="rgba(0, 0, 0, .5)" />
      </TouchableOpacity>
    );
  }
  ...
  renderRetryAppLink() {
    return (
      <View style={styles.retryContainer}>
        <RetryButton onPress={this.onRetryAppLink} style={retryButtonStyle} />
      </View>
    );
  }
